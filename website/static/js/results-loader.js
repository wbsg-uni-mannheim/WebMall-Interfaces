...
function createCostTable(data, InterfaceNames, taskType, completionRates) {
  ...
  const filteredData = data.data.filter(
    (row) =>
      (InterfaceNames.includes(row.Interface) || row.Interface === "AX+Mem") &&
  ...
  filteredData.forEach((row) => {
    ...
    const promptTokens = parseInt(row.prompt_tokens?.replace(/[",]/g, "") || 0);
  ...
  const costData = Object.values(groupedData).map((group) => {
    ...
    const avgCompletionTokens = Math.round(group.completionTokens / taskCount);
    ...
    // Runtime data - only use actual measurements, no estimates
    let runtimeValue = "?"; // Default to unknown

    // TODO: Add actual runtime data from benchmark results when available
    // For now, we don't have reliable runtime measurements for all Interfaces
  ...
  costData.forEach((row) => {
    ...
    tdCompletionRate.textContent = (row.completionRate * 100).toFixed(0) + "%";
  ...
  try {
    ...
    taskTypes.forEach((taskType) => {
      ...
      const table = createResultsTable(
        byTypeData,
        (row) =>
          InterfaceNames.includes(row.Interface) && row.Task_type === taskType,
        InterfaceNames,
        taskType,
      );
    ...
    const categoryContainer = document.getElementById("results-by-category");
  ...
  filteredByType.forEach((row, index) => {
    ...
    if (currentCategory !== lastCategory && lastCategory !== null) {
      ...
      if (currentCategory) {
        ...
        categoryDisplayName =
          categoryRenameMap[categoryDisplayName] || categoryDisplayName;
      }
  ...
  categories.forEach((category) => {
    ...
    InterfaceNames.forEach((Interface) => {
      ...
      const InterfaceData = data.data.filter(
        (row) => row.Interface === Interface && row[categoryField] === category,
      );
    ...
    const axMemData = data.data.filter(
      (row) => row.Interface === "AX+Mem" && row[categoryField] === category,
    );
    ...
    dataRows.forEach((row) => {
      const categoryCell =
        row.cells[
          columns.findIndex(
            (col) =>
              col.toLowerCase().includes("category") || col === "Category",
          )
        ];
      if (categoryCell) {
